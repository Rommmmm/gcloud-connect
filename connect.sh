...
if [ "$(id -u)" = "0" ]; then
   echo -e "\e[1m\e[31mYou are trying to run the script as root, try CTRL+D and run again.\033[0m" 1>&2
   exit 
else
...
echo -e "\e[1m\e[32mPlease select project number\033[0m"
read n
PROJECT=$( echo ${projects[$n]} | cut -d"]" -f2)

##checks if the input is an integer and if its matching any project number
if ! [[ $n = *[[:digit:]]* ]] ;
	then exec >&2; echo -e "\e[1m\e[31merror: Not a number...\033[0m"; run_projects_section;
		elif [[ ($n = 0) ]]; then
		echo -e "\e[1m\e[31mGood bye, exiting...\033[0m"; rm -rf projects.*; exit
		elif [ "$n" -ge "${#projects[@]}" ]
		then
                echo -e "\e[1m\e[31mYou selected non-existing project number...\033[0m";run_projects_section; 
fi
echo -e "\e[1m\e[32mProject selected: $PROJECT\033[0m"

##takes the instances list from selected project and put it in instances.txt
gcloud compute instances list --project $PROJECT | grep "RUNNING" > instances.txt
...
if [ -s instances.lst ]
	then
		...
		echo -e "\e[1m[${#instances[@]}]BACK\033[0m"
		echo -e "\e[1m\e[32mPlease select instance number\033[0m"
		read n2
		INSTANCE=$( echo ${instances[$n2]} | cut -d"]" -f2)
		instance_lines=${#instances[@]}
		if ! [[ $n2 = *[[:digit:]]* ]] ;
        	then exec >&2; echo -e "\e[1m\e[31merror: Not a number...\033[0m"; run_instances_section; 
                	elif [[ ($n2 = 0) ]]; then 
                        echo -e "\e[1m\e[31mGood bye, exiting...\033[0m"; rm -rf instances.*; rm -rf projects.*; exit 
			elif [ "$n2" -gt "${#instances[@]}" ]
                then
                echo -e "\e[1m\e[31mYou selected non-existing project number...\033[0m"; run_instances_section;
			elif [ "$n2" -eq "${#instances[@]}" ]
		then
		echo -e "\e[1m\e[31mSelected: BACK\033[0m"
		run_projects_section
		else
	        ZONE=$(cat instances.txt | grep  "$INSTANCE " | sed -e's/  */ /g' | cut -d" " -f2)
		echo -e "\e[1m\e[32mConnecting to [$INSTANCE] on [$PROJECT]...\033[0m"
		gcloud compute --project "$PROJECT" ssh --zone "$ZONE" "$INSTANCE"; rm -rf instances.*;	 rm -rf projects.*
		fi

	else
	echo -e "\e[1m\e[31mYou selected project with no running instances...\033[0m"; run_projects_section;
fi
}
run_instances_section
rm -rf instances.*
